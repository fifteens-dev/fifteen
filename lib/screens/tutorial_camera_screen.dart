...
/// チュートリアル用カメラ＋写真選択画面 (Component 126)
///
...
class _TutorialCameraScreenState extends State<TutorialCameraScreen>
  ...
  Future<void> _loadAlbumAssets(AssetPathEntity album) async {
    ...
    // 最新写真のサムネイルを取得（カメラページのギャラリーボタン用）
  ...
  Future<void> _loadMorePhotos() async {
    ...
    final assets =
        await _selectedAlbum!.getAssetListRange(start: _currentGalleryPage, end: end);
    if (mounted) {
      setState(() {
  ...
  Future<void> _onShutter() async {
    if (_isTakingPhoto || _cameraController == null || !_isCameraInitialized) return;
    _isTakingPhoto = true;
